...
from src.tm_plotting import TmPlotting
from src.evaluation import Evaluation
from src.RAG import RAG
from src.merge import Merge

from src.doc_selection import DocSelection
from src.speech_preprocess import SpeechPreprocess
...
def get_year_str(task, years, keyword_theme=None):
    ...
    if task == "tm":
        ...
        if not config.doc_selection_parameters["use_keyword_doc_selection"]:
            return "_".join(years)
        else:
...
def get_representative_sents(topic_model, documents, num_docs, topics, texts_path, use_custom=True):
    """
    Finds, returns and saves to file representative sentences for given topics
...
if __name__ == '__main__':
    ...
    if config.speech_preprocessing_parameters["sentencize_speeches"]:
        country = "NL"
        speech_preprocess = SpeechPreprocess(project_root, dataset_name, country, years)
        texts_dict, texts = speech_preprocess.find_texts()
    else:
        ...
        texts_dict, texts = texts_pp.get_texts()  # texts_dict has text title and chunk; texts is only chunks or texts
    ...
    if config.topic_output_parameters['save_topic_words']:
        tm.save_topic_words(topic_model)

    if config.topic_output_parameters['get_represent_sents']:
        text_splits_path = init_folders.get_split_texts_path()
        documents = pd.DataFrame({"Document": texts, "Topic": topic_model.topics_})

        num_docs = config.topic_output_parameters['num_docs']
        topics = config.topic_output_parameters['topics']
    ...
    if task == "tm":
        ...
        rag = RAG(embeddings=embeddings, texts=texts, path=rag_path)

        tm_plotting = TmPlotting(topic_model=topic_model,
        ...
        # Find timestamps
        timestamps, _ = dtm.get_time_stamps(texts_dict)  # Timestamps and chunks
        ...
        print(f'{"Number of text chunks:":<65}{len(texts):>10}')
    ...
    if merge_topics:
        ...
        hierar_topics = merge_obj.get_hierarchical_topics(topic_model, texts_dict)
        topics_to_merge = merge_obj.get_topics_to_merge(hierar_topics)
        print(topics_to_merge)

        topic_model.merge_topics(texts_dict, topics_to_merge)
    ...
    if evaluate_topics:
        ...
        metrics = ["c_v", "c_npmi"]
        coherence_dict = evaluation.calculate_coherence(topic_model, texts_dict, metrics)
