...
class TextCleaning:
    ...
    def __init__(self, project_root, project, year):
        ...
        self.year = year
        self.raw_texts_path = os.path.join(self.input_folder, "raw_texts", year)
        ...
        self.output_folder = os.path.join(self.input_folder, "text_bodies", year)  # Saving cleaned texts in folder text_bodies
        os.makedirs(self.output_folder, exist_ok=True)

        self.regex = config.clean_parameters[f"pattern_{self.project}"]

    def read_clean_raw_texts(self, save_to_folder=True):
        """
        Function to handle reading and cleaning of raw texts
