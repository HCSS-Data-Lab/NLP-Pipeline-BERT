...
class DocSelection:
    ...
    def __init__(self, project_root, dataset_name, years):
        self.project_root = project_root
        self.dataset_name = dataset_name
        self.project_folder = os.path.join(self.project_root, "input", self.dataset_name)
        self.years = years
        self.document_selection_method = config.doc_selection_parameters["doc_selection_method"]
        self.assert_attributes()

    def find_copy_docs(self, keywords=None, keyword_year_str=None):
        """
        Find and copy documents
        """
        if self.document_selection_method == "tfidf":
            # Doc selection with keyword tfidf
            if not keywords or not keyword_year_str:
                raise ValueError("For tfidf document selection, keywords and keyword year str must be defined. ")
            self.select_docs_tfidf(keywords, keyword_year_str)

        elif self.document_selection_method == "search":
            # Doc selection with keyword search
            if not keywords or not keyword_year_str:
                raise ValueError("For tfidf document selection, keywords and keyword year str must be defined. ")
            self.select_docs_search(keywords, keyword_year_str)

        elif self.document_selection_method == "sample":
            # Doc selection with sampling
            self.select_docs_sample()

        else:
            raise ValueError(f"The document selection method: {self.document_selection_method} is undefined. "
                             f"Options are 'tfidf', 'search', or 'sample', define it in config. ")

    def select_docs_tfidf(self, keywords, keyword_year_str):
        """
    ...
    def select_docs_search(self, keywords, keyword_year_str):
        """
        Find docs with a search and match of the keywords
        """
        output_folder = os.path.join(self.project_folder, keyword_year_str, "text_bodies")
        os.makedirs(output_folder, exist_ok=True)
    ...
    def select_docs_sample(self):
        """
        Find docs with a random sample from each year, saving to new folder for each year
        """
        print("Sampling documents...")
        sample_size = config.doc_selection_parameters["sample_size"]
        for year in self.years:
            print(f"Sampling for year: {year}", flush=True)

            # Get samples
            samples = self.sample_docs(year, sample_size)

            # Save to output folder
            output_path = os.path.join(self.project_folder, f"{year}_s{sample_size}", "raw_texts")
    ...
    def sample_docs(self, year, sample_size):
        """
        Sample docs from /input_folder/[year]

        Args:
            year (str): year folder to take samples from
            sample_size (float): sample size in % in range [0, 1]
    ...
    def assert_attributes(self):
        """
        Assert attributes and parameters are correctly defined in config for the selection method
        """
        if self.document_selection_method == "sample":
            if not config.doc_selection_parameters["sample_size"]:   # Assert sample size is defined
                raise ValueError("If the document selection method is 'sample', sample_size should be defined in config. Add it to 'doc_selection_parameters'. ")

        if self.document_selection_method == "tfidf":
            if not config.doc_selection_parameters["tfidf_threshold_type"]:  # Assert tfidf threshold type is defined
                raise ValueError("If the document selection method is 'tfidf', tfidf_threshold_type should be defined in config. "
                                 "Add it to 'doc_selection_parameters'. ")

            if not config.doc_selection_parameters["tfidf_threshold"]:  # Assert tfidf threshold is defined
                raise ValueError("If the document selection method is 'tfidf', tfidf_threshold should be defined in config. "
                                 "Add it to 'doc_selection_parameters'. ")
